...
import { upsertCity } from "@/lib/org";
...
export default async function CitiesPage({ searchParams }: { searchParams: Promise<{ q?: string }> }) {
  ...
  const regionsRes = await supabase.from("region").select("id,name").order("name");
  ...
  const fields: DialogField[] = [
    ...
    { name: "region_id", label: t("org.region"), type: "select", options: regionOpts, allowEmpty: true },
  ];
  ...
  return (
    <div className="mx-auto w-full max-w-5xl space-y-5 ps-12 lg:ps-0">
      ...
      <div className="flex flex-wrap items-center justify-between gap-3">
        <div>
          <h1 className="font-serif text-2xl font-bold tracking-tight text-ink">{t("org.cities")}</h1>
          <p className="text-sm text-muted">{t("org.cities_sub")}</p>
        </div>
        {isAdmin
          ? <EntityDialog title={t("org.add_city")} fields={fields} action={upsertCity} />
      ...
      <form action="/organization/cities" method="get">
        ...
        <table className="w-full text-sm">
          <thead>
            <tr className="border-b border-line bg-cream-deep/40 text-start text-xs uppercase tracking-wide text-muted">
              ...
              <th className="px-4 py-2.5 text-start font-semibold">{t("org.region")}</th>
          ...
          <tbody>
            {rows.length === 0 ? (
              ...
              <tr key={String(r.id)} className="border-b border-line/60 last:border-0 hover:bg-cream/40">
                ...
                <td className="px-4 py-2.5 text-muted">{rel(r.region)}</td>
                {isAdmin && (
                  <td className="px-4 py-2.5 text-end">
                    <EntityDialog title={t("org.add_city")} mode="edit" fields={fields} action={upsertCity}
                      initial={{ id: String(r.id), name: (r.name as string) ?? "", region_id: (r.region_id as string) ?? "" }} />
                  </td>
                )}
              </tr>
            ))}
