...
2. **Phase 1 — pre-commercial core:** **Feature Flags** (first — de-risks every later rollout) → **Global Search** → **Notification Center** → **Bulk Actions** → start **Command Center (role dashboards)**.
3. **Phase 2 — trust & differentiation:** finish **Command Center KPIs**, **Universal Timeline**, **Data Quality Dashboard**, **Favorites**, **Impersonation (read-only)**.
