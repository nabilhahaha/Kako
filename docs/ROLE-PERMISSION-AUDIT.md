...
5. **API / RPC** — `SECURITY DEFINER` functions. **Verified: 27 of 217 `erp_*` functions check
   `erp_user_has_perm`; 82 check apex/tenant identity.** The transactional core
   (`erp_van_sell`, `erp_van_return`, `erp_settle_collection`, `erp_close_day`, the
   `erp_request_*` / `erp_decide_*` governance RPCs) enforces permissions server-side regardless
   of the calling UI.
...
| --- | --- | --- | --- | --- | --- |
| Sales — transact (`sales.sell/collect/return/discount`) | ✅ | ✅ | ⚠️ (sell/invoices ✅; POS/orders/returns pages ❌ no guard) | 🔒 | ✅ (`erp_van_sell`/`erp_settle_collection`/`erp_van_return` check perm) |
...
> **Bottom line:** tenant isolation is solid (RLS everywhere) and the transactional core enforces
> permissions in the RPC. The gaps are (a) within-tenant **read** screens guarded by module+RLS
> only, and (b) high-impact **financial/inventory** permissions that live in the DB but not the
> code union, so their server-side checks must be individually verified. Every *critical* action
> (sell, collect, return, close day, reconcile, approve requests, post/approve vouchers) should
> have an RPC-level `erp_user_has_perm` check — confirmed for the FMCG core; **to be verified for
> voucher-approve, invoice-cancel, payment-writeoff, change-requests, trade-spend.**
