-- ============================================================================
-- 0311: FMCG customer onboarding model + credit/payment request kinds
-- ----------------------------------------------------------------------------
-- Expands the governed customer-request model (0310) into the full FMCG
-- onboarding request and adds two more kinds (credit_limit, payment_terms) on
-- the SAME infrastructure. Salesmen never write master data; the change is
-- applied server-side ONLY on approval. New rules enforced server-side:
--   • new_customer requires name, mobile, owner, activity, city, district, GPS
--   • a Storefront photo (erp_attachments doc_type='storefront') is REQUIRED to
--     approve a new_customer (data-quality gate)
--   • on approval the customer is created ASSIGNED (salesman + route) so it is
--     never an orphan record
-- Additive + reversible.
-- ============================================================================
...
-- ── Raise a customer request (salesman) — expanded validation ────────────────
...
BEGIN
  ...
  END IF;
  IF p_kind NOT IN ('new_customer','data_update','gps_correction','credit_limit','payment_terms') THEN
...
-- ── Decide + APPLY a customer request (approver) — full onboarding + assign ──
...
BEGIN
  ...
  IF req.kind = 'new_customer' THEN
    -- Data-quality gate: a Storefront photo must be attached to the request.
    IF NOT EXISTS (
      SELECT 1 FROM erp_attachments
       WHERE entity='customer_request' AND record_id = p_request_id::text
         AND doc_type='storefront' AND deleted_at IS NULL
    ) THEN
    ...
    v_custom := jsonb_strip_nulls(jsonb_build_object(
      ...
      'activity', NULLIF(p->>'activity',''),
      'classification', NULLIF(p->>'classification',''),
      'competitor', NULLIF(p->>'competitor',''),
      'expected_monthly_sales', NULLIF(p->>'expected_monthly_sales',''),
      'secondary_mobile', NULLIF(p->>'mobile2',''),
      'national_address_short', NULLIF(p->>'na_short',''),
      'building_number', NULLIF(p->>'building_no',''),
      'additional_number', NULLIF(p->>'additional_no',''),
      'postal_code', NULLIF(p->>'postal_code','')
    ));
    ...
    VALUES (v_co, NULLIF(p->>'branch_id','')::uuid, req.salesman_id, NULLIF(p->>'route_id','')::uuid, v_code,
       ...
       COALESCE(NULLIF(p->>'requested_credit_limit','')::numeric, 0),
       NULLIF(p->>'requested_terms','')::int,
