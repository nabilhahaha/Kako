...
import { Plus, Pencil, Loader2, X, Scissors } from 'lucide-react';
import { formatCurrency } from '@/lib/utils';
import { upsertService } from '../actions';

export interface Service { id: string; name: string; price: number; duration_min: number; is_active: boolean }
...
export function ServicesManager({ services }: { services: Service[] }) {
  ...
  return (
    <div className="space-y-4">
      <Button onClick={() => setEditing('new')}><Plus className="h-4 w-4" /> خدمة جديدة</Button>
      {editing && (
        <Card><CardContent className="pt-6">
          <form onSubmit={onSubmit} className="space-y-4">
            {editing !== 'new' && <input type="hidden" name="id" value={editing.id} />}
            <div className="grid gap-3 sm:grid-cols-4">
              <div className="space-y-1 sm:col-span-2"><Label>اسم الخدمة *</Label><Input name="name" required defaultValue={editing !== 'new' ? editing.name : ''} placeholder="قص / صبغة / مكواة" /></div>
              ...
              <div className="space-y-1"><Label>المدة (دقيقة)</Label><Input name="duration_min" type="number" min={5} step={5} dir="ltr" defaultValue={editing !== 'new' ? editing.duration_min : 30} /></div>
      ...
      <Card><CardContent className="p-0">
        {services.length === 0 ? (
          <div className="flex flex-col items-center gap-2 p-8 text-center text-muted-foreground"><Scissors className="h-8 w-8" /><p>لا توجد خدمات بعد.</p></div>
        ) : (
          <table className="w-full text-sm">
            <thead className="border-b bg-secondary/50 text-muted-foreground"><tr>
              <th className="p-3 text-right font-medium">الخدمة</th><th className="p-3 text-center font-medium">المدة</th><th className="p-3 text-center font-medium">السعر</th><th className="p-3 text-center font-medium">الحالة</th><th className="p-3"></th>
            </tr></thead>
            <tbody>
              {services.map((s) => (
                <tr key={s.id} className="border-b">
                  <td className="p-3 font-medium">{s.name}</td>
                  <td className="p-3 text-center text-muted-foreground tabular-nums">{s.duration_min} د</td>
