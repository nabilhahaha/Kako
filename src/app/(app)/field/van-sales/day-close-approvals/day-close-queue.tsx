...
import { decideDayCloseStage, type PendingDayCloseRow } from '@/lib/van-sales/day-close-server';
  ...
  async function decide(r: PendingDayCloseRow, decision: 'approve' | 'reject') {
    ...
    try {
      const res = await decideDayCloseStage({
        ...
        variance: variance.trim() ? Number(variance) : undefined,
      });
      if (!res.ok) { toast.error(res.error ?? dl('error')); return; }
      setRows((list) => list.filter((x) => x.id !== r.id));
      setOpen(null); setReason(''); setComment(''); setVariance('');
        ...
        return (
          <Card key={r.id}>
            <CardContent className="space-y-3 pt-5">
              ...
              {canAct ? (
                <>
                  <button type="button" onClick={() => { setOpen(isOpen ? null : r.id); setReason(''); setComment(''); setVariance(''); }} className="flex items-center gap-1 text-xs font-medium text-primary">
                    ...
                    <div className="space-y-3 rounded-md border bg-secondary/20 p-3">
                      {isReconcileOrSettle && (
                        <div className="space-y-1.5">
                          <Label className="text-xs">{r.stage === 'settle' ? dl('cashVariance') : dl('stockVariance')} <span className="text-muted-foreground">({dl('optional')})</span></Label>
                          <Input type="number" inputMode="decimal" value={variance} onChange={(e) => setVariance(e.target.value)} placeholder="0" />
                        </div>
                      )}
                      <div className="space-y-1.5">
