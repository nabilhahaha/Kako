...
function NavigateButton({ lat, lng }: { lat: number | null; lng: number | null }) {
  const { t } = useI18n();
  const [open, setOpen] = useState(false);
  if (!hasValidCoords(lat, lng)) {
    return <Button size="sm" variant="outline" disabled><Navigation className="h-4 w-4" /> {t('vanSales.smartNext.navigate')}</Button>;
  }
