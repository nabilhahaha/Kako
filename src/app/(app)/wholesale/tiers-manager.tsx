...
export function TiersManager({ tiers }: { tiers: Tier[] }) {
  ...
  function onSubmit(e: React.FormEvent<HTMLFormElement>) {
    ...
    startTransition(async () => {
      ...
      toast.success(editing === 'new' ? t('wholesale.toastTierAdded') : t('wholesale.toastTierUpdated')); setEditing(null); router.refresh();
    });
  }

  return (
    <div className="space-y-4">
      <Button onClick={() => setEditing('new')}><Plus className="h-4 w-4" /> {t('wholesale.btnNewTier')}</Button>
      {editing && (
        <Card><CardContent className="pt-6">
          <form onSubmit={onSubmit} className="space-y-4">
            {editing !== 'new' && <input type="hidden" name="id" value={editing.id} />}
            <div className="grid gap-3 sm:grid-cols-3">
              <div className="space-y-1 sm:col-span-2"><Label>{t('wholesale.labelTierName')}</Label><Input name="name" required defaultValue={editing !== 'new' ? editing.name : ''} placeholder={t('wholesale.placeholderTierName')} /></div>
              <div className="space-y-1"><Label>{t('wholesale.labelTierSort')}</Label><Input name="sort" type="number" dir="ltr" defaultValue={editing !== 'new' ? editing.sort : 0} /></div>
            </div>
            <div className="flex gap-2"><Button type="submit" disabled={pending}>{pending && <Loader2 className="h-4 w-4 animate-spin" />} {t('wholesale.btnSave')}</Button><Button type="button" variant="outline" onClick={() => setEditing(null)}><X className="h-4 w-4" /> {t('wholesale.btnCancel')}</Button></div>
          </form>
        </CardContent></Card>
      )}
