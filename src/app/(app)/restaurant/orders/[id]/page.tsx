...
export default async function OrderEditorPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const { data: order } = await supabase
    ...
    .select('id, order_type, status, customer_name, customer_phone, customer_address, delivery_fee, notes, table:erp_restaurant_tables(name)')
    .eq('id', id)
    .maybeSingle();
  if (!order) notFound();
  const o = order as unknown as {
    id: string; order_type: string; status: string; customer_name: string | null; customer_phone: string | null;
    customer_address: string | null; delivery_fee: number; notes: string | null; table: { name: string } | null;
  };
  ...
  const editorOrder: EditorOrder = {
    ...
    delivery_fee: Number(o.delivery_fee || 0), notes: o.notes, table_name: o.table?.name ?? null,
  };
