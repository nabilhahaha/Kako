...
export async function updateForm(input: {
  ...
  const { error } = await supabase.from('erp_form_definitions').update({
    ...
    workflow_key: input.workflowKey?.trim() || null, status: input.status, updated_at: new Date().toISOString(),
  }).eq('id', input.id);
