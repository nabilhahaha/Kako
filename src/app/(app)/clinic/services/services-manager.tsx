...
export function ServicesManager({ services }: { services: Service[] }) {
  ...
  function onSubmit(e: React.FormEvent<HTMLFormElement>) {
    ...
    startTransition(async () => {
      ...
      toast.success(editing === 'new' ? 'تمت إضافة الخدمة' : 'تم تحديث الخدمة');
      setEditing(null);
      router.refresh();
    });
  }
          ...
          <CardContent className="pt-6">
            <form onSubmit={onSubmit} className="space-y-4">
              ...
              <div className="grid gap-3 sm:grid-cols-3">
                <div className="space-y-1"><Label>اسم الخدمة *</Label><Input name="name" required defaultValue={editing !== 'new' ? editing.name : ''} placeholder="كشف / استشارة / إجراء" /></div>
        ...
        <CardContent className="p-0">
          {services.length === 0 ? (
            <div className="flex flex-col items-center gap-2 p-8 text-center text-muted-foreground">
              <Tags className="h-8 w-8" /><p>لا توجد خدمات بعد. أضف أول خدمة بسعرها.</p>
            </div>
          ) : (
            <table className="w-full text-sm">
              <thead className="border-b bg-secondary/50 text-muted-foreground">
                <tr>
                  <th className="p-3 text-right font-medium">الخدمة</th>
