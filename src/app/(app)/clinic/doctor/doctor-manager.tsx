    ...
    <div className="space-y-4">
      ...
      {doctors.length > 1 && (
        <div className="flex items-center gap-2">
          <span className="text-sm text-muted-foreground">عرض طابور:</span>
          <select value={filter} onChange={(e) => setFilter(e.target.value)} className={`${selectCls} w-auto`}>
            {meIsDoctor && <option value={currentDoctorId}>طابوري</option>}
            <option value="all">كل الأطباء</option>
            {doctors.filter((d) => d.id !== currentDoctorId).map((d) => (
              <option key={d.id} value={d.id}>د. {d.full_name || d.email}</option>
            ))}
          </select>
        </div>
      )}
